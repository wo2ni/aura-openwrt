...
#kylin os version
ifeq (${KYLIN_KVER},)
  KYLIN_KVER := $(shell echo ${KVER}| cut -s -d'-' -f2 | cut -s -d'.' -f1)
  EXTRA_CFLAGS += -DKYLIN_KERNEL_REVISON=${KYLIN_KVER}
endif
# minimum_kver_check
#
...
cmd_initrd := $(shell \
                if which update-initramfs > /dev/null 2>&1 ; then \
                    ...
                    if [ ${KVER_CODE} -lt $(call get_kvercode,2,7,0) ]; then \
                        echo "dracut --force"; \
                    else \
                        echo "dracut --force -L 0"; \
                    fi \
                fi )
...
# Default to using updates/drivers/net/ethernet/wangxun/ path, since depmod since
...
export INSTALL_MOD_DIR ?= updates/drivers/net/ethernet/wangxun/${DRIVER}
