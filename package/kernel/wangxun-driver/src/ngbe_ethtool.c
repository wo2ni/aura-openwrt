		    ...
		    struct ethtool_link_ksettings *cmd)
{
	...
	if ((hw->phy.type == ngbe_phy_m88e1512_sfi) ||
	    ...
	    (hw->phy.type == ngbe_phy_internal_yt8521s_sfi)) {
#ifndef HAVE_NOT_SUPPORTED_1000baseX_Full
		if (ethtool_link_ksettings_test_link_mode(cmd, supported, 1000baseX_Full))
			ethtool_link_ksettings_add_link_mode(cmd, advertising,
							     1000baseX_Full);
#endif
		if (ethtool_link_ksettings_test_link_mode(cmd, supported, 100baseT_Full))
			ethtool_link_ksettings_add_link_mode(cmd, advertising,
							     100baseT_Full);
		ethtool_link_ksettings_zero_link_mode(cmd, advertising);
		...
		if (hw->phy.autoneg_advertised & NGBE_LINK_SPEED_1GB_FULL) {
			...
			if (ethtool_link_ksettings_test_link_mode
				(cmd, supported, 1000baseX_Full))
				ethtool_link_ksettings_add_link_mode
					(cmd, advertising, 1000baseX_Full);
			else
#endif
				ethtool_link_ksettings_add_link_mode
		...
		if (hw->phy.autoneg_advertised) {
			if (hw->phy.autoneg_advertised & NGBE_LINK_SPEED_1GB_FULL) {
				if (ethtool_link_ksettings_test_link_mode(cmd, supported, 1000baseKX_Full)) {
					ethtool_link_ksettings_add_link_mode(cmd, advertising, 
								 1000baseKX_Full);
				} else if (ethtool_link_ksettings_test_link_mode(cmd, supported, 1000baseT_Full)){
					ethtool_link_ksettings_add_link_mode(cmd, advertising, 
								 1000baseT_Full);
				} else {
#ifndef HAVE_NOT_SUPPORTED_1000baseX_Full
...
static int ngbe_phys_id(struct net_device *netdev, u32 data)
{
	...
	if (adapter->hw.phy.type == ngbe_phy_yt8521s_sfi ||
		...
		adapter->hw.phy.type == ngbe_phy_m88e1512_sfi) {		
		TCALL(hw, phy.ops.write_reg_mdi, 22, 0, 3);
		TCALL(hw, phy.ops.read_reg_mdi, 16, 0, (u16*)&led_reg);
		for (i = 0; i < (data * 1000); i += 400) {
			TCALL(hw, phy.ops.write_reg_mdi, 16, 0, (adapter->led_reg & ~0xF) | 0x9);
			msleep_interruptible(200);
			TCALL(hw, phy.ops.write_reg_mdi, 16, 0, (adapter->led_reg & ~0xF) | 0x8);
		...
		TCALL(hw, phy.ops.write_reg_mdi, 16, 0, led_reg);
	} else {	
