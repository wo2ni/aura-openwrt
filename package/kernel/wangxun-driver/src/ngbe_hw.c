...
int check_image_version(struct ngbe_hw *hw, const u8 *data)
{
	u32 image_v = 0x0;
	u32 f_chip_v = 0x0;
	u8 rdata_2;
	...
	u32 f_sub_id;
	u8 wol = 0, ncsi = 0;
	printk("===========check_image_version============\n");

	//read image version
	image_v = data[0x13a] | data[0x13b] << 8 |
		  data[0x13c] << 16 | data[0x13d] << 24;
	printk("check_image_version=image_v: %x\n", image_v);
	...
	if ((f_sub_id & 0x4000) == 0x4000)
		wol = 1;
	printk("=2=ncsi : %x - wol : %x\n", ncsi, wol);

	rdata_2 = data[0xfffd8];
	printk("check_image_version=rdata_2-fffdc: %x\n", rdata_2);
	rdata_3 = data[0xbc];
	printk("check_image_version=rdata_3-bc: %x\n", rdata_3);
	rdata_4 = data[0x3c];
	printk("check_image_version=rdata_4-3c: %x\n", rdata_4);
				...
				const u8 *data, u32 size)
{
	...
	printk("Checking chip/image version .......\n");
	printk("The image chip_v is %c\n", image_v);
	printk("The nic chip_v is %c\n", chip_v);
	if (chip_v != image_v)
	{
		printk("====The Gigabit image is not match the Gigabit card (chip version)====\n");
	...
	for (k=0; k<(512/4); k++)
		num[k] = ngbe_flash_read_dword(hw, 0xfe000 + k*4);
	...
	for (i=0; i<size/4; i++) {
		...
		if (read_data != 0xffffffff && !skip) {
			...
			if (status) {
				...
				read_data = ngbe_flash_read_dword(hw, i*4);
	...
	flash_write_dword(hw, MAC_ADDR2_WORD1_OFFSET_1G, (mac_addr2_dword1_t | 0x80000000));//lan2
	flash_write_dword(hw, MAC_ADDR3_WORD0_OFFSET_1G, mac_addr3_dword0_t);
	flash_write_dword(hw, MAC_ADDR3_WORD1_OFFSET_1G, (mac_addr3_dword1_t | 0x80000000));//lan3
	...
	if (*link_up) {
		if ( speed_sta == 0x28) {
			...
			*speed = NGBE_LINK_SPEED_100_FULL;
			...
			if (NGBE_LINK_RETRY == 1) {
				if ((hw->restart_an == 0) && (hw->autoneg_err_cnt > 0)) {
					value = NGBE_MDI_PHY_RESTART_AN | NGBE_MDI_PHY_ANE;
					TCALL(hw, phy.ops.write_reg, 0, 0, value);
					e_info(hw, "autoneg_err_cnt is %d, restart AN.\n", hw->autoneg_err_cnt);
					hw->restart_an = 1;
				}
			}
			e_info(hw, "autoneg_err_cnt is %d\n", hw->autoneg_err_cnt);
...
bool ngbe_is_lldp(struct ngbe_hw *hw)
{
	struct ngbe_adapter *adapter = hw->back;
	struct pci_dev *pdev = adapter->pdev;
	u32 tmp = 0;
	u32 lldp_flash_data = 0 ;
	u32 i = 0;

	for (; i < 0x1000 / sizeof(u32); i++) {
        tmp = ngbe_flash_read_dword(hw, NGBE_LLDP_REG + i * 4);
        if (tmp != 0xffffffff) {
            lldp_flash_data = tmp;
        } else {
            break;
	...
	return (lldp_flash_data & BIT(PCI_FUNC(pdev->devfn)));
}
